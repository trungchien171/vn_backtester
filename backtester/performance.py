...
def total_return(final_portfolio_value, initial_cap):
    return (final_portfolio_value/initial_cap) - 1

def annualized_return(total_return, num_of_days):
    return np.power((1+total_return), 252/num_of_days) - 1

def annualized_volatility(daily_returns):
    return daily_returns.std() * np.sqrt(252)

def sharpe_ratio(annualized_return, annualized_volatility, risk_free_rate=0):
    try:
        return (annualized_return - risk_free_rate) / annualized_volatility
    except ZeroDivisionError:
        return 0
def sortino_ratio(daily_returns, annualized_return, risk_free_rate=0):
